...
```mermaid
graph LR
  idea["Idea"] --> G0["Gate 0: Idea Screen"]
  G0 --> S1["Stage 1: Scoping"]
  S1 --> G1["Gate 1: Second Screen"]
  G1 --> S2["Stage 2: Build Business Case"]
  S2 --> G2["Gate 2: Go to Development"]
  G2 --> S3["Stage 3: Development"]
  S3 --> G3["Gate 3: Go to Testing"]
  G3 --> S4["Stage 4: Testing & Validation"]
  S4 --> G4["Gate 4: Go to Launch"]
  G4 --> S5["Stage 5: Launch"]
  S5 --> PIR["Post-Launch Review"]
```
