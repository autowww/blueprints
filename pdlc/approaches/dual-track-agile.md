...
```mermaid
graph LR
  subgraph discoveryTrack ["Discovery Track (Product Trio)"]
  ...
  subgraph deliveryTrack ["Delivery Track (Engineering Team)"]
    specify["Specify stories"]
    build["Build"]
    verify["Verify"]
    release["Release"]
  end

  validate -->|"validated ideas"| specify
  release -->|"usage data + feedback"| interview

  interview --> hypothesize --> prototype --> validate
  specify --> build --> verify --> release
```
