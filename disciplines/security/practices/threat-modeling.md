...
flowchart LR
  ...
  DFD --> TID[Threat identification STRIDE per element]
  TID --> RR[Risk rating DREAD qualitative CVSS]
  RR --> MIT[Mitigation mapping controls requirements]
```
...
sequenceDiagram
  ...
  Facilitator->>Team: Scope system and trust assumptions
  Team->>Team: Build diagram DFD or architecture
  Team->>Sec: Review boundaries and assets
  Facilitator->>Team: Enumerate threats STRIDE LINDDUN etc
  Team->>Team: Assess likelihood and impact
  Sec->>Team: Propose mitigations and owners
  Facilitator->>Team: Document model threats mitigations
  Team->>Team: Track as requirements and follow up
```
...
```mermaid
flowchart TD
  IM[Initial model at design] --> IMPL[Implementation and controls]
  IMPL --> RV[Revalidation tests and review]
  RV --> CH{Architecture change}
  CH -->|Yes| UPD[Update model and requirements]
  CH -->|No| RV
  UPD --> IMPL
```
