...
```mermaid
flowchart TB
  SAST["SAST — static code broad automated base"]
  SCA["SCA — dependencies and licenses"]
  DAST["DAST — dynamic against running systems"]
  PT["Penetration testing — expert manual depth"]
  RT["Red teaming — adversary simulation peak manual"]
  SAST --> SCA
  SCA --> DAST
  DAST --> PT
  PT --> RT
...
```mermaid
flowchart LR
  C[Code commit] --> SAST[SAST]
  C --> SCA[SCA]
  SAST --> B[Build]
  SCA --> B
  B --> D[DAST on staging]
  D --> R[Aggregate report]
  R --> G{Gate decision}
  G -->|Pass| Ship[Promote or deploy]
  G -->|Fail| Fix[Fix or waive with record]
```
...
```mermaid
flowchart TD
  A[New finding] --> B{Exploitable in our context?}
  B -->|No / FP| FP[Document close or defer]
  B -->|Yes| C{In production or prod path?}
  C -->|No| D[Schedule fix before prod]
  C -->|Yes| E{Public exploit or active use?}
  E -->|Yes| F[Critical path escalate]
  E -->|No| G[Rate severity CVSS + context]
  F --> H[Assign SLA and owner]
  G --> H
  H --> I[Track to remediation]
```
