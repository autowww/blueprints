...
flowchart TB
  subgraph sources["Data sources"]
    ...
    B[Billing]
    V[VoC / surveys]
    O[Outcomes / success plans]
  end

  subgraph engine["Scoring engine"]
    N[Normalize & join]
    R[Rules / weights / model]
    SC[Account score]
  end
...
```mermaid
flowchart TD
  D[Detect signal change] --> T[Assign / update tier]
  T --> I[Intervention: playbook + owner]
  I --> M[Monitor response]
  M --> L[Learn: playbook effectiveness]
  L --> D
```
