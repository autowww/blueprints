...
```mermaid
flowchart TB
  ACQ[Acquisition<br/>visitors / signups]
  ACT[Activation<br/>aha completed]
  RET[Retention<br/>repeat use / renewal]
  REV[Revenue<br/>paid / expanded]
  REF[Referral<br/>invites / shares]
  ACQ -->|"c₁"| ACT
  ACT -->|"c₂"| RET
  RET -->|"c₃"| REV
  REV -->|"c₄"| REF
...
```mermaid
flowchart LR
  ID[Ideate] --> PR[Prioritize<br/>ICE / RICE]
  PR --> EX[Experiment]
  EX --> ME[Measure]
  ME --> LE[Learn]
  LE --> ID
