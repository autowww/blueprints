...
flowchart TB
  ...
  subgraph products [Data products]
    P1[Product APIs / datasets]
    P2[Product APIs / datasets]
    P3[Product APIs / datasets]
  end

  subgraph platform [Self-serve data platform]
    S[Storage / compute templates]
    O[Orchestration & CI]
    C[Catalog & lineage]
    M[Monitoring & quality gates]
  end

  subgraph gov [Federated governance mesh]
    G[Global policies & standards]
...
flowchart LR
  subgraph dp [Data product]
    IN[Input ports<br/>sources / contracts]
    PIPE[Pipeline &<br/>transformations]
    OUT[Output ports<br/>tables / streams / APIs]
    DISC[Discovery:<br/>catalog + docs]
    OBS[Observability:<br/>metrics / lineage]
    SLO[SLOs &<br/>support model]
  end
...
```mermaid
flowchart TD
  A[Discover consumer need] --> B[Define contract<br/>schema + SLAs]
  B --> C[Build pipeline<br/>with quality gates]
  C --> D[Publish to catalog]
  D --> E[Monitor SLOs & usage]
  E --> F{Change needed?}
  F -->|Yes| G[Evolve with versioning<br/>deprecation policy]
  G --> E
  F -->|Retire| H[Deprecate & archive]
```
