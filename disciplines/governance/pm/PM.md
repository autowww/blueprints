...
```mermaid
graph LR
  IN["Initiating"] --> PL["Planning"]
  PL --> EX["Executing"]
  EX --> MC["Monitoring & Controlling"]
  MC --> CL["Closing"]
  MC -.->|"corrective action"| PL
  MC -.->|"corrective action"| EX
  PL -.->|"progressive elaboration"| EX
```
