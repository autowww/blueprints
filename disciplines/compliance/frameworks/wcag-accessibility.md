...
```mermaid
flowchart LR
  LA[Legal / market analysis] --> SC[Define scope: URLs flows components]
  SC --> AUTO[Automated testing CI + scans]
  AUTO --> MAN[Manual: keyboard zoom reflow]
  MAN --> AT[Assistive technology testing]
  AT --> REM[Remediation backlog]
  REM --> VPAT[VPAT / ACR]
  VPAT --> MON[Monitoring regression]
```
...
```mermaid
flowchart TD
  Q[Selling to US federal agencies?] -->|Yes| S508[Section 508 / Rev 508 — WCAG 2.0 AA baseline + other 508 criteria]
  Q -->|No| E[Offering services in EU in EAA scope?] -->|Yes| EAA[EN 301 549 / WCAG-aligned — plan for EAA]
  E -->|No| ADA[US public-facing commercial site/app?] -->|Counsel says Title III risk| WCAGAA[Treat WCAG 2.x AA as target]
  ADA -->|No / low risk jurisdiction| BP[Voluntary best practice — still WCAG AA for quality]
```
