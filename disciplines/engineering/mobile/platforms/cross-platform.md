...
```mermaid
flowchart TB
  subgraph shared["Shared core (domain, data, networking)"]
    SC[Shared Kotlin / Dart / TS]
  end
  subgraph abstr["Platform abstraction"]
    PA[Storage, crypto, analytics ports]
  end
  subgraph native["Native modules / plugins"]
    NM[iOS Swift / Android Kotlin]
  end
