...
sequenceDiagram
  ...
  U->>L: Local change
  L->>Q: Enqueue operation
  Q->>S: Dequeue when online
  S->>R: Push change
  R-->>S: 409 / conflict payload
  S->>S: Detect conflict
  alt LWW
    S->>L: Apply winning version
  else Merge / CRDT
    S->>L: Merge structures
  else Manual
    S->>U: Present conflict UI
    U->>L: Resolve and persist
  end
```
