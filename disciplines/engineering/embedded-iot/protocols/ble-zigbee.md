...
flowchart TB
  P[Profile / Use case]
  S1[Service: Battery]
  S2[Service: Environmental Sensing]
  C1[Characteristic: Level]
  C2[Characteristic: Temperature]
  D1[Descriptor: CCCD]
  P --> S1
  P --> S2
  S1 --> C1
...
```mermaid
flowchart TD
  A[Need short-range wireless?] --> B{Phone-centric / user phone as UI?}
  B -->|Yes| C[Strong BLE candidate]
  B -->|No| D{Mesh + many battery nodes?}
  D -->|Yes| E{Zigbee / Thread / Matter ecosystem?}
  E --> F[Matter on Thread or Wi-Fi]
  D -->|No| G{High throughput / video?}
  G -->|Yes| H[Wi-Fi or wired]
  G -->|No| I{WAN / km range?}
  I -->|Yes| J[Not short-range: LoRa / cellular]
  I -->|No| K[BLE or simple star 802.15.4]
```
