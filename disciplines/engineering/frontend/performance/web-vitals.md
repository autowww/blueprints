...
```mermaid
timeline
  title Page load lifecycle (simplified) and metric windows
  section Navigation
    TTFB : Server response begins
  section Paint
    FCP : First content paints
    LCP : Largest content paints (LCP candidate window)
  section Interaction
    INP : After input, until next paint (throughout page life)
  section Stability
    CLS : Accumulates shifts during session
```
...
```mermaid
flowchart TD
  M[Measure] --> I[Identify bottleneck]
  I --> O[Optimize]
  O --> V[Validate]
  V --> Mon[Monitor]
  Mon --> M
