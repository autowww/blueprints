...
if [[ $# -lt 1 ]] || [[ "${1:-}" == -* ]]; then
  ...
  echo "Optional coding footprint rule: add --with-code-footprint-rules to sync/diff/status" >&2
  echo "Optional cost-aware planning rules: add --with-cost-tiering-rules to sync/diff/status" >&2
