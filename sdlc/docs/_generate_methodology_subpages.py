...
MERMAID_BLOCK = """
          <section class="mt-4" id="flow-preview">
            <h2 class="font-display h5 fw-semibold">Flow preview (diagram)</h2>
            <p class="text-body-secondary small">Rendered below when JavaScript runs (Mermaid). The Markdown source has the same diagrams for GitHub.</p>
            <div class="border rounded-3 bg-white p-3 shadow-sm"><div class="mermaid small">{mermaid}</div></div>
          </section>
"""
...
METHODS = [
    {
        ...
        "pages": [
            ...
            (
                ...
                "Sprint lifecycle, planning flow, daily loop, DoD gate, stakeholder feedback — as <strong class=\"text-body\">Mermaid</strong> maps in Markdown; preview on this page.",
            ),
        ],
        "mermaid": """flowchart LR
  R[Refinement] --> P[Sprint Planning]
  P --> D[Daily loop]
  D --> V[Sprint Review]
  V --> RT[Retrospective]
  RT --> R""",
  ...
  U --> R[Release]""",
    },
    {
        ...
        "mermaid": """flowchart LR
  IP[Iteration planning] --> DEV[Build TDD/CI]
  DEV --> ACC[Acceptance]
  ACC --> DEMO[Demo]
  DEMO --> RT[Retro]
  RT --> IP""",
...
def page_html(
    ...
    extra = MERMAID_SCRIPT if include_mermaid else ""
    mermaid_sec = ""
    if include_mermaid:
        mermaid_sec = MERMAID_BLOCK.format(mermaid=esc(m["mermaid"]))
    body = f"""
...
# Full methodology subchapters are generated by build_methodology_chapters.py (tables + Mermaid).
    ...
    for m in METHODS:
        for page_id, title_html, md_file, blurb in m["pages"]:
            ...
            if fn in SKIP_STUB_GENERATOR:
                ...
                continue
            include = page_id == "process"
            html = page_html(m, page_id, title_html, md_file, blurb, include)
