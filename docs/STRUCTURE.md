...
**Scalability:** Keep **`blueprints/docs/`** at **repository root** (frozen). Mutable content lives under **`docs/product/`** next to `docs/requirements/`, `docs/architecture/`, etc.
...
| Layer | Role |
|-------|------|
| **`blueprints/docs/`** | Frozen IA + templates (this package). |
| **`docs/product/`** | Living product functional content. |
...
- **Monorepo:** One `blueprints/docs/` at repo root; optional `docs/product/<product-name>/` if needed.
